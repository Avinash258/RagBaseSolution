# Playwright Docs — docs locators

Source: https://playwright.dev/docs/locators

Locators | PlaywrightSkip to main contentOn this page
# Locators
...
awaitexpect(page.getByText('Welcome, John!')).toBeVisible();
`
...
`awaitexpect(page.getByRole('heading',{name:'Sign up'})).toBeVisible();
...
`awaitexpect(page.getByText('Welcome, John',{exact:true})).toBeVisible();
`

Match with a regular expression:

`awaitexpect(page.getByText(/welcome, [A-Za-z]+$/i)).toBeVisible();
`
note
Matching by text always normalizes whitespace, even with exact match. For example, it turns multiple spaces into one, turns line breaks into spaces and ignores leading and trailing whitespace.When to use text locators
...
For example, consider the following DOM structure.
http://localhost:3000
`<imgalt="playwright logo"src="/img/playwright-logo.svg"width="100"/>
`
...
`<spantitle='Issues count'>25 issues</span>
`

You can check the issues count after locating it by the title text:

`awaitexpect(page.getByTitle('Issues count')).toHaveText('25 issues');
`
...
`<buttondata-testid="directions">Itinéraire</button>
`
...
http://localhost:3000Itinéraire
`<buttondata-pw="directions">Itinéraire</button>
`
...
<div>Title</div>
  #shadow-root
<divid=inner-details>Details</div>
</x-details>
`
...
`awaitexpect(page.locator('x-details')).toContainText('Details');
`
...
`// 5 in-stock items
awaitexpect(page.getByRole('listitem').filter({hasNotText:'Out of stock'})).toHaveCount(5);
`
...
awaitexpect(product).toHaveCount(1);
`
...
awaitexpect(newEmail.or(dialog).first()).toBeVisible();
...
`awaitexpect(page.getByRole('listitem')).toHaveCount(3);
`
...
`for(const row ofawait page.getByRole('listitem').all())
