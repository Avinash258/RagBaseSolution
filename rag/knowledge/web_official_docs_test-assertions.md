# Playwright Docs — docs test-assertions

Source: https://playwright.dev/docs/test-assertions

Assertions | PlaywrightSkip to main contentOn this page
# Assertions
...
`awaitexpect(page.getByTestId('status')).toHaveText('Submitted');
`
...
awaitexpect(locator).not.toContainText('some text');
`
...
`awaitexpect(page.getByText('Name'),'should be logged in').toBeVisible();
`
...
awaitslowExpect(locator).toHaveText('Submit');

// Always do soft assertions.
const softExpect = expect.configure({soft:true});
awaitsoftExpect(locator).toHaveText('Submit');
`
...
exportconst expect = baseExpect.extend({
asynctoHaveAmount(locator:Locator,expected: number, options?:{ timeout?: number }){
const assertionName ='toHaveAmount';
letpass: boolean;
letmatcherResult: any;
try{
...
test('amount',async()=>{
awaitexpect(page.locator('.cart')).toHaveAmount(4);
});
`
...
exportconst expect =mergeExpects(dbExpect, a11yExpect);
...
awaitexpect(database).toHaveDatabaseUser('admin');
});
`
