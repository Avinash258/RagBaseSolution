# Playwright Docs — docs mock

Source: https://playwright.dev/docs/mock

Mock APIs | PlaywrightSkip to main contentOn this page
# Mock APIs
...
await page.route('*/**/api/v1/fruits',asyncroute=>{
...
awaitexpect(page.getByText('Strawberry')).toBeVisible();
});
`
...
awaitexpect(page.getByText('Loquat',{exact:true})).toBeVisible();
});
`
...
Alternatively, you can also record HAR files by using the recordHar option in browser.newContext() when creating a browser context. This allows you to capture all network traffic for the entire context until the context is closed.
...
awaitexpect(page.getByText('Playwright',{exact:true})).toBeVisible();
});
`
