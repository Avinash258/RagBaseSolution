# Playwright Docs — docs test-fixtures

Source: https://playwright.dev/docs/test-fixtures

Fixtures | PlaywrightSkip to main contentOn this page
# Fixtures
...
awaitexpect(page).toHaveTitle(/Playwright/);
});
`
...
browserBrowserBrowsers are shared across tests to optimize resources. Learn how to configure browsers.
...
Click to expand the code for the `TodoPage`todo-page.ts
...
asyncgoto(){
awaitthis.page.goto('https://demo.playwright.dev/todomvc/');
}

asyncaddToDo(text: string){
awaitthis.inputBox.fill(text);
awaitthis.inputBox.press('Enter');
}

asyncremove(text: string){
...
asyncremoveAll(){
while((awaitthis.todoItems.count())>0){
awaitthis.todoItems.first().hover();
awaitthis.todoItems.getByLabel('Delete').first().click();
}
}
}
`
...
const todoPage =newTodoPage(page);
await todoPage.goto();
await todoPage.addToDo('item1');
await todoPage.addToDo('item2');
awaituse(todoPage);
...
Click to expand the code for the `TodoPage` and `SettingsPage`todo-page.ts
`import type {Page,Locator}from'@playwright/test';
...
SettingsPage is similar:settings-page.ts
`import type {Page}from'@playwright/test';

exportclassSettingsPage{
constructor(public readonly page:Page){
}

asyncswitchToDarkMode(){
// ...
}
}
`
...
awaitexpect(page.getByTestId('todo-title')).toContainText(['something nice']);
});
`
...
const cookie =awaitgetAuthCookie();
awaituse({cookies:[cookie]});
},
});
`
...
exportconst test = base.extend<{},{account:Account}>({
...
awaitexpect(page.getByTestId('result')).toHaveText('Success');
// Do not forget to cleanup.
await page.close();

// Use the account value.
awaituse({ username, password });
},{scope:'worker'}],
...
awaitexpect(page.getByTestId('userinfo')).toHaveText(username);
...
exportconst test = base.extend<{saveLogs:void}>({
...
exportconst test = base.extend<MyOptions&MyFixtures>({
...
exportdefault defineConfig<MyOptions>({
projects:[
{
name:'shopping',
...
// workerFixture setup...
awaituse('workerFixture');
// workerFixture teardown...
},{scope:'worker'}],

autoWorkerFixture:[async({ browser })=>{
// autoWorkerFixture setup...
awaituse('autoWorkerFixture');
// autoWorkerFixture teardown...
},{scope:'worker',auto:true}],

testFixture:[async({ page, workerFixture })=>{
// testFixture setup...
awaituse('testFixture');
// testFixture teardown...
},{scope:'test'}],

autoTestFixture:[async()=>{
// autoTestFixture setup...
awaituse('autoTestFixture');
// autoTestFixture teardown...
},{scope:'test',auto:true}],

unusedFixture:[async({ page })=>{
// unusedFixture setup...
awaituse('unusedFixture');
...
exportconst test =mergeTests(dbTest, a11yTest);
`
test.spec.ts
...
exportconst test = base.extend<{forEachTest:void}>({
...
exportconst test = base.extend<{},{forEachWorker:void}>({
